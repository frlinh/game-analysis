...
Once the Database was pulled locally, cleanup begins with the dropping of columns unneccessary to the Analysis of the file, as well as null values. This allows the code to run cleanly and without issues. The columns that were kept were decided in the group meetings, dependent upon the purpose of the Champion mastery.
...
After the cleanup, we create a new dataframe of Champions and their average number of points, total number of points, and total number of summons. 
...
We sort the Champions and create a new dataframe to show the stats per level. Afterwards, we get the averages of those stats/elements.
...
With this, we will be able to see which Champions are easy, intermediate, and difficult. Now that the analysis has been done based all all the summoner data and Champion stats, we'll be able to run our machine learning models.
...
### Random Forest
Our final try was with a random forest model. Upon our first try, we see a very high accuracy rate: 
...
With concern of overfitting, we try other options such as changing the random state and/or training size. 

## Conclusion & Results
The Random Forest is the best model to use because it had the highest accuracy score. The other models all had an accuracy score below 0.372. 
